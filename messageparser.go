...
func ParseMessage(buf []byte) (Message, *Delimiters, error) {
	...
	if len(buf) < 8 {
		return nil, nil, ErrTooShort(errors.Errorf("message must be at least eight bytes long; instead was %d", len(buf)))
	}
	...
	if !bytes.HasPrefix(buf, []byte("MSH")) {
		return nil, nil, ErrInvalidHeader(errors.Errorf("expected message to begin with MSH; instead found %q", buf[0:3]))
	}
	...
	if fs == cs || fs == rs || fs == ec || fs == ss || cs == rs || cs == ec || cs == ss || rs == ec || rs == ss || ec == ss {
		return nil, nil, ErrInvalidHeader(errors.Errorf("all control characters must be unique"))
	}
	...
	if len(buf) > 8 && buf[8] != fs {
		return nil, nil, ErrInvalidHeader(errors.Errorf("invalid character found after header content; expected \\x%02x but got \\x%02x", fs, buf[8]))
	}
